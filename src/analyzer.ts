...
export interface Signal {
  name: string;
  score: number; // -1 to +1, negative = human, positive = ai
  evidence: string;
}

export class AICodeAnalyzer {
  private aiScores: { [key: string]: number } = {};
  private humanScores: { [key: string]: number } = {};
  ...
  analyze(
    ...
    linesAdded: number,
    linesDeleted: number,
    changeDuration: number // seconds between first and last file change
  ): AnalysisResult {
    this.reset();
    ...
    let classification: 'ai' | 'human' | 'mixed' = 'mixed';
   ...
   */
  private analyzeCommitMessage(message: string): Signal {
   ...
   */
  private analyzeBulkChanges(linesAdded: number, filesChanged: number): Signal {
    let score = 0;
    let evidence = '';
   ...
   */
  private analyzeVelocity(linesAdded: number, durationSeconds: number): Signal {
    let score = 0;
    let evidence = '';
   ...
   */
  private analyzeFileCoordination(filesChanged: number, linesAdded: number): Signal {
    let score = 0;
    let evidence = '';
   ...
   */
  private analyzeChangePattern(linesAdded: number, linesDeleted: number): Signal {
    let score = 0;
    let evidence = '';
   ...
   */
  private buildReasoning(signals: Signal[], classification: string): string {
   ...
   */
  private recordScore(signalName: string, score: number): void {
    if (score > 0) {
